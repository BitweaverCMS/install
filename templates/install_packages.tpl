...
{else}
	...
	{jstabs tab=0}
		...
		{if !$first_install}
			{jstab title="Uninstall/Reinstall"}
					...
					<div class="control-group column-group gutters">
						{forminput}
							<input type="submit" class="ink-button" name="submit_packages" value="Uninstall/Reinstall Packages" onclick="return confirm( 'Are you sure you want to uninstall/reinstall the selected packages?' );" />
						{/forminput}
					</div>
	...
	{if !$first_install}
			...
			<div class="control-group column-group gutters">
				{forminput}
					<input type="submit" class="ink-button" name="cancel" value="Skip this stage" />
				{/forminput}
			</div>
	{/if}
	{/form}
